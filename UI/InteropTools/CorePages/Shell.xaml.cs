...
using InteropTools.ShellPages.Process;
...
namespace InteropTools
{
    public sealed partial class Shell : Page
    {
        ...
        int Width0 = 0;
        int Width1 = 125; //250; //720;
        int Width2 = 1024; //1024;
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            if (e.Parameter.GetType() == typeof(ViewLifetimeControl))
            {
                ...
                Titlebar.UpdateLayout();//Titlebar.UpdateTitle(); // ?
            }
        }
        ...
        private void ThisViewControl_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            ...
            {
                Titlebar.UpdateLayout();//.UpdateTitle();
            }
        }
        ...
        public Shell()
        {
            ...
            // Analytics Info Detection: Mobile or not mobile? =)
            bool ismobile = AnalyticsInfo.VersionInfo.DeviceFamily.ToLower() == "windows.mobile";

            //RnD : choose another namespace

            // Rnd
            vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "Welcome Page 3.0", PageType = typeof(Pages.Core.WelcomePage), GroupIcon = "", GroupName = "Core" });
            vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "Welcome Page 2.2", PageType = typeof(ShellPages.Core.WelcomePage), GroupIcon = "", GroupName = "Core" });


            //vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "Providers", PageType = typeof(ProviderPage), GroupIcon = "", GroupName = "Core" });

            if (!ismobile)
                vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "Crash bum bang!", PageType = typeof(Pages.SplashScreen), GroupIcon = "", GroupName = "Splash" });

            if (ismobile)
                vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "Your Device", PageType = typeof(ShellPages.Core.YourDevicePage), GroupIcon = "", GroupName = "Core" });
            
            if (ismobile)
                vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "Store", PageType = typeof(ShellPages.Store.StorePage), GroupIcon = "", GroupName = "Core" });


            vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "App List Control", PageType = typeof(ShellPages.AppManager.AppListControl), GroupIcon = "", GroupName = "Applications" });
            
            //if (ismobile)
            vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "Application Manager", PageType = typeof(ShellPages.AppManager.AppManagerPage), GroupIcon = "", GroupName = "Applications" });

            vm.TopItems.Add(new NavigationItem { Icon = "", DisplayName = "App Product Page", PageType = typeof(ShellPages.AppManager.AppProductPage), GroupIcon = "", GroupName = "Applications" });
        ...
        private void UpdateBackButtonVisibility()
        {
            ...
            Titlebar.Visibility = (Visibility)visibility; // BackButtonVisibility
        }
        ...
        private void Titlebar_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            ...
            if (Window.Current.Bounds.Width < 300) //< 720)
                padding.Top = 0;
        ...
        // Магия по изменению размеров того-сего =)
        private void Redraw()
        {
            ...
            //
            if (Window.Current.Bounds.Width >= Width2) // 1024
            {
                // DisplayMode - CompactInline
                SplitView.DisplayMode = SplitViewDisplayMode.CompactInline;

                // ?
                if (previouswidthcap != Width2) // 1024
                {
                ...
                // OpenPaneLength = 320
                SplitView.OpenPaneLength = 320; //320;
                ...
                previouswidthcap = Width2; // set widthcap flag 
            }
            else if (Window.Current.Bounds.Width >= Width1) // 720
            {
                SplitView.DisplayMode = SplitViewDisplayMode.CompactOverlay;

                if (previouswidthcap != Width1) // 720
                {
                ...
                SplitView.OpenPaneLength = 256; //256;
                NarrowHeaderGrid.Visibility = Visibility.Collapsed;
                PaneHeader.Visibility = Visibility.Visible;
                ...
                previouswidthcap = Width1; // // set widthcap flag 
            }
            else
            {
                //
        ...
        private void HandleSplitViewItems()
        {
            ...
            if (Window.Current.Bounds.Width >= Width1 && !(SplitView.IsPaneOpen))
            {
                TopList.Visibility = Visibility.Visible;
